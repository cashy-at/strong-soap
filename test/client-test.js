...
describe("SOAP Client", function () {
  ...
  it('should set binding style to "document" by default if not explicitly set in WSDL, per SOAP spec', function (done) {
    soap.createClient(
      __dirname + "/wsdl/binding_document.wsdl",
      function (err, client) {
        assert.ok(client);
        assert.ok(!err);

        assert.ok(
          client.wsdl.definitions.bindings.mySoapBinding.style === "document"
        );
        done();
      }
    );
  ...
  describe("Create a client from a wsdl preloaded in the options", function () {
    ...
    beforeEach(function (done) {
      ...
      fs.readFile(
        __dirname + "/wsdl/default_namespace.wsdl",
        "utf8",
        function (err, defaultNamespaceWsdlContents) {
          if (err) {
            done(err);
          } else {
            var options = {
            ...
            defaultNamespaceWsdl = new WSDL(
              defaultNamespaceWsdlContents,
              undefined,
              {}
            );
  ...
  describe("Headers in request and last response", function () {
    ...
    before(function (done) {
      server = http
        .createServer(function (req, res) {
          var status_value =
            req.headers["test-header"] === "test" ? "pass" : "fail";
    ...
    it(
      "should append port " +
        "to the Host header on for a request to a service on that port",
      function (done) {
        soap.createClient(
          __dirname + "/wsdl/default_namespace.wsdl",
          function (err, client) {
    ...
    it("should not return error in the call and return the json in body", function (done) {
      soap.createClient(
        __dirname + "/wsdl/json_response.wsdl",
        function (err, client) {
          assert.ok(client);
          assert.ok(!err);
    ...
    it("should add proper headers for soap12", function (done) {
      soap.createClient(
        __dirname + "/wsdl/default_namespace_soap12.wsdl",
        function (err, client) {
          assert.ok(client);
          assert.ok(!err);
    ...
    it("should allow calling the method as a promise with only args", function (done) {
      soap.createClient(
        ...
        function (err, client) {
          ...
          assert.ok(!err);
          client
            .MyOperation({ Request: "temp request" })
            .then(({ result, envelope }) => {
              assert.ok(!err);
              assert.ok(result);
              assert.ok(result === "temp response");
      ...
      soap.createClient(
        ...
        function (err, client) {
          ...
          client
            ...
            .then(({ result, envelope, soapHeader }) => {
              assert.ok(!err);
              assert.ok(result);
              assert.ok(result === "temp response");
              assert.ok(envelope.toString() === rawBody);
        ...
        soap.createClient(
          ...
          function (err, client) {
            ...
            client.MyOperation({}, function (err, result, body) {
              ...
              assert.strictEqual(
                err.message,
                "faultcode: Test faultstring: test error detail: test detail"
              );
              assert.ok(result);
              assert.ok(body);
              done();
