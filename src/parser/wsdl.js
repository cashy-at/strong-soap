...
class WSDL {
  ...
  _initializeOptions(options) {
    ...
    else this.options.ignoreBaseNameSpaces = this.ignoreBaseNameSpaces;
      ...
      if (wsdl.definitions instanceof Definitions) {
        ...
        if (undefined in wsdl.definitions.schemas) {
          if (include.namespace != null) {
            ...
            wsdl.definitions.schemas[include.namespace] =
              wsdl.definitions.schemas[undefined];
        ...
        _.mergeWith(self.definitions, wsdl.definitions, function (a, b) {
          ...
          return a instanceof Schema
            ? a.merge(b, include.type === "include")
            : undefined;
        });
      } else {
        self.definitions.schemas[
    ...
    p.onopentag = function (node) {
      ...
      if (top) {
        ...
        if (name === "definitions") {
          ...
          throw new Error(
            g.f("Unexpected root element of {{WSDL}} or include")
          );
        }
      }
    };
  ...
  static open(uri, options, callback) {
    ...
    debug(
      "wsdl open. request_headers: %j request_options: %j",
      request_headers,
      request_options
    );
    var wsdl;
      ...
      httpClient.request(
        ...
        function (err, response, definition) {
          if (err) {
            ...
            const reqUri = `${response.req.protocol}//${
              response.req.getHeaders().host
            }${response.req.path}`;

            const wsdlUri =
              _.get(response, "request.uri.href") || reqUri || uri;
  ...
  static openSync(uri, options) {
    ...
    if (fromCache && !fromCache.isLoaded) {
      ...
      throw (
        uri +
        " was not found in the cache. For loadSync() calls all schemas must be preloaded into the cache"
      );
    }
