...
class Client extends Base {
  ...
  _invoke(operation, args, location, callback, options, extraHeaders) {
    ...
    debug(
      "client request. operation: %s args: %j options: %j extraHeaders: %j",
      operation.name,
      args,
      options,
      extraHeaders
    ...
    var soapVersion =
      this.wsdl.options.forceSoapVersion || operation.soapVersion;
    ...
    debug(
      "client request. soapNsURI: %s soapNsPrefix: %s ",
      soapNsURI,
      soapNsPrefix
    );
    ...
    dynamicSoapHeaders = _.merge(
      _.pick(extraHeaders, dynamicSoapHeaderProperty),
    ...
    var xmlHandler =
      this.xmlHandler || new XMLHandler(this.wsdl.schemas, options);
    ...
    if (
      ...
      self.httpClient.options.prettyPrint !== undefined
    ) {
    ...
    req = self.httpClient.request(
      ...
      function (err, response, body) {
        ...
        if (err) {
          ...
          if (
            body.indexOf("<soap:Fault>") > -1 ||
            body.indexOf("<Fault>") > -1
          ) {
          ...
          try {
            debugDetail(
              "client response. outputEnvDescriptor: %j",
              outputEnvDescriptor
            );
          ...
          debug(
            "client response. result: %j body: %j obj.Header: %j",
            result,
            body,
            obj.Header
          );
